...
import { ArrowLeft, Check, AlertCircle, Save, X, Trash2 } from 'lucide-react';
      ...
      try {
        ...
        if (res.ok && data.success && data.product) {
          ...
          setFormData({
            ...
            category: prod.category || 'Face Wash',
            ...
            discountPrice: prod.discountPrice !== undefined ? String(prod.discountPrice) : '',
        ...
        setError('A network connection error occurred while retrieving details.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setError(data.message || 'Failed to update product details.');
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center space-y-4">
        ...
        <span className="text-xs text-slate-505 font-medium">Loading product records...</span>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <Link
            ...
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-505 hover:text-[#3B5F43] transition-colors"
          >
        ...
        {/* Title */}
        <div>
          <h1 className="font-serif text-2xl sm:text-3xl font-bold text-slate-800">Edit Product Listing</h1>
          <p className="text-xs text-slate-505 mt-1">
            Update pricing configurations, adjust stock levels, or rewrite details for the formula.
          </p>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="name" className="block text-xs font-semibold text-slate-700 mb-1.5">
                    Product Name <span className="text-rose-550">*</span>
                  </label>
                  <input
                    type="text"
                    id="name"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="productId" className="block text-xs font-semibold text-slate-700 mb-1.5">
                      SKU Barcode ID <span className="text-rose-550">*</span>
                    </label>
                    <input
                      type="text"
                      id="productId"
                ...
                <div>
                  <label htmlFor="description" className="block text-xs font-semibold text-slate-700 mb-1.5">
                    Product Description <span className="text-rose-550">*</span>
                  </label>
                  <textarea
                    id="description"
                    name="description"
            ...
            <div className="md:col-span-4 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label htmlFor="weight" className="block text-xs font-semibold text-slate-700 mb-1.5">
                      Net Weight / Volume <span className="text-rose-550">*</span>
                    </label>
                    <input
                      type="text"
                      id="weight"
                  ...
                  <div>
                    <label htmlFor="price" className="block text-xs font-semibold text-slate-700 mb-1.5">
                      Regular Price (₹) <span className="text-rose-550">*</span>
                    </label>
                    <input
                      type="number"
                      id="price"
                  ...
                  <div>
                    <label htmlFor="stock" className="block text-xs font-semibold text-slate-700 mb-1.5">
                      Stock Quantity <span className="text-rose-550">*</span>
                    </label>
                    <input
                      type="number"
                      id="stock"
                    ...
                    <div key={idx} className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-2 border border-slate-200 hover:bg-slate-50 text-slate-400 hover:text-slate-650 rounded-lg transition-colors cursor-pointer"
                        >
                          &times;
                        </button>
                      </div>
              ...
              <button
                ...
                className={`w-full py-4 rounded-xl text-sm font-semibold text-white bg-[#3B5F43] hover:bg-[#2A4430] shadow-sm hover:shadow transition-all flex items-center justify-center gap-2 cursor-pointer ${
                  saving || success ? 'opacity-70 pointer-events-none' : ''
                }`}
              >
                {saving ? (
                  <>
